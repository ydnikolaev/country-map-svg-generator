...
profiles: []
concerns: []
inputs: ["DISC-006", "ARCH-001", "DEC-001", "AM-001"]
---
...
ARCH-001 sets separate size/byte budgets. AM-001 makes the intrinsic viewBox
natural to each country's projected proportions by default and reserves fixed
width-by-height canvases for explicit distortion-free containment.
...
| REQ-10 | Catalog and browser evidence distinguishes intrinsic SVG aspect from CSS slot size: wide, tall and near-square `tight` assets retain natural viewBoxes, while tiny/huge portrait/landscape/square `contain` assets use one uniform scale, remain centered and never distort geography. |
...
| VAL-7 | REQ-2, REQ-6, REQ-8, REQ-10 | inspect and render wide (`RU`), tall (`CL`/`AR`) and near-square (`AU`) natural assets plus tiny, huge, portrait, landscape and square containing frames | full-catalog+browser E2E; intrinsic ratio, CSS sizing, uniform transform, centering, containment and no-distortion mutations have teeth | P4 / W4 complete |

## Acceptance criteria

| ID | Covers | Criterion | Evidence |
| --- | --- | --- | --- |
| AC-1 | US-1, REQ-1, REQ-2, REQ-10 | Copy-ready folder contains the complete card/hero catalog; natural intrinsic proportions and explicit containing frames pass structural/budget/no-distortion gates without post-processing. | full-catalog receipt |
| AC-2 | US-2, REQ-4, REQ-5, REQ-6, REQ-7, REQ-8, REQ-10 | Documented inline hero, mask card and image card examples preserve intrinsic proportions and work in browser fixtures across themes, slot shapes and reduced motion. | browser E2E |
...
AM-001 is accepted and binding: client and catalog evidence must distinguish
natural intrinsic SVG proportions from explicit distortion-free containing frames.
