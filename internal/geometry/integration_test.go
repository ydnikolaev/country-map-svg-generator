...
func TestRepresentativeNaturalRatiosAndArbitraryFrames(t *testing.T) {
	...
	// The card is fitted to the silhouette it draws, so an entity's aspect is
	// the aspect of its drawn geometry rather than of every component it claims.
	// Chile is the case that settled this: its UN geometry spans 43.0 degrees of
	// longitude across Easter Island, Salas y Gomez and Juan Fernandez against
	// 13.5 for the mainland, and those Pacific components are not drawn at card
	// scale — so a Chile card is tall and narrow, which is what this assertion
	// has always said.
	//
	// It is worth recording that this assertion was briefly rewritten to demand
	// the opposite while the frame was fitted to the whole claim. The original
	// expectation was right and the pipeline was wrong; fitting to the candidate
	// restored it, and Chile now measures 0.29 rather than 0.98.
